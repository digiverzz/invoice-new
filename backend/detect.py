        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    # custom.append(xyxy2xywh(torch.tensor(xyxy).view(1, 4)) / gn).view(-1).tolist()
...
def start(weights,source,img_size=640,conf_thres=0.75,iou_thres=0.75):
    ...
    opt={"weights":weights,"source":source,"img_size":img_size,
         "conf_thres":conf_thres,"iou_thres":iou_thres,"device":'',
         'view_img':False,'save_txt':True,'save_conf':False,
         "nosave":False,"classes":None,'agnostic_nms':False,
         'augment':False,'update':False,'project':'runs/detect','name':'exp','exist_ok':False,'no_trace':True}
